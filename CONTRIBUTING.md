# Contributing to singularity-installer

## Development setup

```bash
git clone https://github.com/singularityos-lab/singularity-installer
cd singularity-installer
meson setup build
ninja -C build
```
...
- Keep files focused: one primary class per `.vala` file, named after the class
  (e.g. `InstallerWindow` -> `window.vala`). Redundant suffixes in the
